...
public class HuggingFaceService {
    ...
    public boolean isTextAppropriate(String text) {
        try {
            ...
            List<List<Map<String, Object>>> outerList = response.getBody();

            if (outerList != null && !outerList.isEmpty()) {
                List<Map<String, Object>> resultList = outerList.get(0);

                for (Map<String, Object> labelScore : resultList) {
                    String label = (String) labelScore.get("label");
                    Double score = ((Number) labelScore.get("score")).doubleValue();

                    if ((label.equalsIgnoreCase("toxicity") || label.equalsIgnoreCase("hate") || label.equalsIgnoreCase("threat"))
                            && score > 0.7) {
                        return false;
                    }
                }
    ...
    public boolean containsScamKeywords(String text) {
        List<String> scamKeywords = Arrays.asList(
                ...
                "whatsapp’tan ulaş", "tapu sonra", "hemen yatır", "banka yok"
        );
