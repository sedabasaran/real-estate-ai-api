...
public class ImageServiceImpl implements ImageService {
	...
	public ImageResponseDTO addImage(ImageRequestDTO requestDTO) {
		Property property = propertyRepository.findById(requestDTO.getPropertyId()).orElseThrow(
				() -> new PropertyNotFoundException("Property not found with id: " + requestDTO.getPropertyId()));
	...
	public List<ImageResponseDTO> getImagesByPropertyId(Long propertyId) {
		return imageRepository.findByPropertyId(propertyId).stream().map(imageMapper::toResponseDTO)
